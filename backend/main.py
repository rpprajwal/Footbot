...
def enforce_formation_and_build_teams(players, team_count, formation, team_size, subs):
    ...
    for pos in positions:
        remaining.extend(by_pos[pos])

    for t in range(team_count):
        while len(teams[t]["subs"]) < subs and remaining:
            teams[t]["subs"].append(remaining.pop())

    idx = 0
    while remaining:
        teams[idx % team_count]["subs"].append(remaining.pop())
        idx += 1

    for t in range(team_count):
        while len(teams[t]["starters"]) + len(teams[t]["subs"]) > team_size and teams[t]["subs"]:
            teams[t]["subs"].pop()

    return teams


def fitness_for_teams(teams, formation, team_size):
    team_scores = []
    pos_expected = formation
    sizes = [len(t["starters"]) + len(t["subs"]) for t in teams]
    avg_size = sum(sizes) / len(sizes)
    size_penalty = sum(abs(s - avg_size) for s in sizes)
    for t in teams:
        total = sum(player_rating(p) for p in t["starters"]) + 0.5 * sum(player_rating(p) for p in t["subs"])
        team_scores.append(total)
    avg_score = sum(team_scores) / len(team_scores)
    skill_penalty = sum(abs(s - avg_score) for s in team_scores)
    pos_penalty = 0
    for t in teams:
        counts = {pos: 0 for pos in positions}
        for p in t["starters"]:
            counts[p.get("position")] += 1
        pos_penalty += abs(counts["Goalkeeper"] - pos_expected.get("Goalkeeper", 1)) * 10
        for pos in ["Defender", "Midfielder", "Forward"]:
            pos_penalty += abs(counts[pos] - pos_expected.get(pos, 0))
    formation_violation = pos_penalty * 2
    return skill_penalty + formation_violation + size_penalty


def genetic_multi_split(players, team_count, formation, team_size, subs=3, generations=200):
    best = None
    best_score = float("inf")
    for _ in range(generations):
        teams = enforce_formation_and_build_teams(players, team_count, formation, team_size, subs)
        f = fitness_for_teams(teams, formation, team_size)
        if f < best_score:
            best = copy.deepcopy(teams)
            best_score = f
    out = []
    for t in best:
        combined = list(t["starters"]) + list(t["subs"]) if t else []
        if combined:
            if not any(p.get("captain") for p in combined):
                captain = max(combined, key=lambda x: player_rating(x))
                captain["captain"] = True
        out.append({"players": combined, "starters": t["starters"], "subs": t["subs"]})
    return out


def logistic_prob(a, b, k=1.0):
    try:
        return 1.0 / (1.0 + math.exp(-(a - b) / k))
